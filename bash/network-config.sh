...
myinterfacename=$(ip a | awk '/: e/{gsub(/:/,"");print $2}')
echo "INTERFACE NAME:"$myinterfacename
mylanaddress=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
echo "LAN ADDRESS:"$mylanaddress
mylanhostname=$(getent hosts $(hostname -I)| awk '{print $2}')
echo "LAN HOSTNAME:"$mylanhostname
myexternalip=$((curl -s icanhazip.com) | awk 'print $2')
echo "EXTERNAL IP:"$myexternalip
myexternalname=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
echo "EXTERNAL NAME:"$myexternalname
myrouterip=$(ip r s default| awk '{print $3}')
echo "ROUTER IP:"$myrouterip
myroutername=$(route |grep "default" | awk '(print $2)')
echo "ROUTER NAME:"$myroutername
